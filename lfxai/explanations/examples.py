...
class InfluenceFunctions(ExampleBasedExplainer, ABC):
    ...
    def attribute(
        ...
        **kwargs,
        ...
        loss = [
            self.loss_f(self.X_train[idx:idx + 1], self.model(self.X_train[idx:idx + 1]))
            for idx in train_idx
        ]

        grads = [
            stack_torch_tensors(
                torch.autograd.grad(loss[k], self.model.encoder.parameters(), create_graph=True))
        ...
        for _ in tqdm(range(recursion_depth), unit="recursion", leave=False):
            ...
            hvps_ = [
                stack_torch_tensors(hessian_vector_product(sampled_loss, self.model,
                                                           [IHVP_prev[k]]))
        ...
        IHVP_ = [IHVP_[k] / (scale * NUM_SAMPLES) for k in range(len(train_idx))
                ]  # Rescale Hessian-Vector products
        IHVP_ = torch.stack(IHVP_, dim=0).reshape(
            (len(train_idx), -1))  # Make a tensor (len(train_idx), n_params)
        ...
        test_grads = [
            stack_torch_tensors(
                torch.autograd.grad(test_loss[k],
        ...
        **kwargs,
        ...
        for test_idx, (x_test, _) in enumerate(tqdm(test_loader, unit="example", leave=False)):
            ...
            test_grad = (stack_torch_tensors(
                torch.autograd.grad(test_loss, self.model.encoder.parameters(),
                                    create_graph=True)).detach().clone())
        ...
        for train_idx, (x_train, _) in enumerate(tqdm(train_loader, unit="example", leave=False)):
            ...
            grad = stack_torch_tensors(
                torch.autograd.grad(loss, self.model.encoder.parameters(), create_graph=True))
        ...
        for test_idx, train_idx in product(
            ...
            attribution[test_idx, train_idx] = (torch.dot(test_grad.flatten(),
                                                          ihvp.flatten()).detach().clone().cpu())
        return attribution
    ...
    def attribute_loader(
        ...
        **kwargs,
        ...
        for checkpoint_number, checkpoint_file in enumerate(
            ...
            self.model.load_state_dict(torch.load(checkpoint_file), strict=False)
            for train_idx, (x_train, _) in enumerate(tqdm(train_loader, unit="example",
                                                          leave=False)):
                x_train = x_train.to(device)
                train_loss = self.loss_f(x_train, self.model(x_train))
                train_grad = stack_torch_tensors(
                    torch.autograd.grad(train_loss,
                                        self.model.encoder.parameters(),
                                        create_graph=True))
                torch.save(
                    train_grad.detach().cpu(),
                    self.save_dir / f"train_checkpoint{checkpoint_number}_grad{train_idx}.pt",
                )
            for test_idx, (x_test, _) in enumerate(tqdm(test_loader, unit="example", leave=False)):
                x_test = x_test.to(device)
                test_loss = self.loss_f(x_test, self.model(x_test))
                test_grad = stack_torch_tensors(
                    torch.autograd.grad(test_loss,
                                        self.model.encoder.parameters(),
                                        create_graph=True))
                torch.save(
                    test_grad.detach().cpu(),
                    self.save_dir / f"test_checkpoint{checkpoint_number}_grad{test_idx}.pt",
                )
            for test_idx, train_idx in product(
                range(len(test_loader)), range(len(train_loader)), leave=False
            ):
                train_grad = torch.load(self.save_dir /
                                        f"train_checkpoint{checkpoint_number}_grad{train_idx}.pt")
                test_grad = torch.load(self.save_dir /
                                       f"test_checkpoint{checkpoint_number}_grad{test_idx}.pt")
                attribution[test_idx, train_idx] += torch.dot(train_grad.flatten(),
                                                              test_grad.flatten())
        return attribution
