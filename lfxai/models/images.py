    ...
    def test_epoch(self, device: torch.device, dataloader: torch.utils.data.DataLoader):
        ...
        with torch.no_grad():
            for image_batch, label_batch in dataloader:
                ...
                test_loss.append(loss.cpu().numpy())
                test_acc.append(
                    torch.count_nonzero(pred_batch == label_batch).cpu().numpy() / len(image_batch))
...
class VarEncoderMnist(nn.Module):
    ...
    def __init__(self, c: int = 64, latent_dims: int = 10):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=c, kernel_size=4, stride=2,
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0),
                   -1)  # flatten batch of multi-channel feature maps to a batch of feature vectors
        x_mu = self.fc_mu(x)
...
class VarDecoderMnist(nn.Module):
    ...
    def __init__(self, c: int = 64, latent_dims: int = 10):
        ...
        self.conv2 = nn.ConvTranspose2d(in_channels=c * 2,
                                        out_channels=c,
                                        kernel_size=4,
                                        stride=2,
                                        padding=1)
        self.conv1 = nn.ConvTranspose2d(in_channels=c,
                                        out_channels=1,
                                        kernel_size=4,
                                        stride=2,
                                        padding=1)
...
class BetaVaeMnist(nn.Module):
    ...
    def test_epoch(self, device: torch.device,
                   dataloader: torch.utils.data.DataLoader) -> np.ndarray:
        self.eval()
        test_loss = []
        with torch.no_grad():
            for image_batch, _ in dataloader:
...
class BetaTcVaeMnist(nn.Module):
    ...
    def loss(
        ...
        importance_weights = (torch.Tensor(batch_size,
                                           batch_size).fill_(1 / (batch_size - 1)).to(x.device))
...
class DecoderBurgess(nn.Module):
    ...
    def __init__(self, img_size, latent_dim=10):
        ...
        # If input image is 64x64 do fourth convolution
        if self.img_size[1] == self.img_size[2] == 64:
            self.convT_64 = nn.ConvTranspose2d(hid_channels,
                                               hid_channels,
                                               kernel_size,
                                               **cnn_kwargs)
...
class SimCLR(nn.Module):
    ...
    def __init__(self, base_encoder, projection_dim=128):
        ...
        self.encoder = base_encoder(
            pretrained=True)  # load model from torchvision.models without pretrained weights.
    ...
    def fit(self, args: DictConfig, device: torch.device) -> None:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for x, y in train_bar:
                ...
                train_bar.set_description(f"Train epoch {epoch}, SimCLR loss: {loss_meter.avg:.4f}")

            # save checkpoint very log_interval epochs
            if epoch >= args.log_interval and epoch % args.log_interval == 0:
                logger.info("==> Save checkpoint. Train epoch {}, SimCLR loss: {:.4f}".format(
